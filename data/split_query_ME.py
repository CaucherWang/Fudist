...
if __name__ == "__main__":
    ...
    if select == 'hnsw':
        me_delta0_path = os.path.join(source, dataset, f'{dataset}_me_delta0_recall{target_recall}.ibin_hnsw_ef{efConstruction}_M{M}{idx_postfix}')
    elif select == 'kgraph':
        me_delta0_path = os.path.join(source, dataset, f'{dataset}_me_delta0_recall{target_recall}.ibin')
    else:
        raise ValueError(f'select {select} not supported')
    # recalls = []
    ...
    me_delta0_indexes = np.argsort(me_delta0)
    npart = 10
    ...
    for i in range(npart):
        qindexes = me_delta0_indexes[i*part_size:(i+1)*part_size]
        query_splits.append(Q[qindexes])
        qpath = os.path.join(source, dataset, f'{dataset}_query.fvecs_me_delta0_level{i}')
        gtpath = os.path.join(source, dataset, f'{dataset}_groundtruth.ivecs_me_delta0_level{i}')
