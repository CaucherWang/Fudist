...
if __name__ == "__main__":
    ...
    hnsw = read_hnsw_index_unaligned(hnsw_path, Q.shape[1])
    ...
    # assert KG < G.shape[1]
    three_sets = [np.arange(Q.shape[0])]
    ...
    #             sum_recall += recall
    #         # print()
    #         efs_recalls[efs] = np.array(recalls)
    ...
    # write_ibin_simple(os.path.join(source, dataset, f'{dataset}_recall-160.ibin'), efs_recalls[160])
    recalls = read_ibin_simple(os.path.join(source, dataset, f'{dataset}_recall-160.ibin'))
    print(np.corrcoef(recalls, q_lengths))
    res = np.vstack([recalls, q_lengths, np.arange(Q.shape[0])]).T
    # sort res by res by recalls in ascent ORDER
    ...
    for i in range(200):
        gt = GT[int(res[i][2])][:1]
        for gt_id in gt:
            k_occurs.append(n_rknn[gt_id])
            lens.append(lengths[gt_id])
    print(f'avg_k_occurs: {np.average(k_occurs)}, avg_lens: {np.average(lens)}')
    # import matplotlib.pyplot as plt
