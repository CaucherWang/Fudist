...
if __name__ == '__main__':
    ...
    for i in range(data_size):
        ...
        neighbor_list.append([id2label[nei] for nei in neighbors])
    ...
    total_overlap = 0
    for i in range(data_size):
        length = len(neighbor_list[i])
        overlap = np.intersect1d(gt_I[i][:32], neighbor_list[i]) 
        total_overlap += overlap.shape[0]
    print(f"total_overlap = {total_overlap}, total_cnt = {total_cnt}, ratio = {total_overlap / total_cnt}")
