...
if __name__ == "__main__":
    ...
    hnsw_ind_path = os.path.join(source, dataset, f'{dataset}_hnsw_ef{efConstruction}_M{M}_ind_{KG}.ibin{idx_postfix}')
    ...
    q_id = 9766
    nn1 = GT[9766][0]
    nn1_in_neighbors = revG[nn1]
    nn1_in_nei_dist = [euclidean_distance(Q[q_id], X[nei]) for nei in nn1_in_neighbors]
    print(nn1_in_nei_dist)
    ...
    # plt.xlabel('indegree of HNSW')
    # plt.ylabel('visited_times')
    # # plt.xlim(0, 1200)
    # # plt.xscale('log')
    # # plt.yscale('log')
    # # plt.title(f'{dataset} indegree correlation')
    # plt.savefig(f'./figures/{dataset}-Hnsw-visit-hub-correlation.png')
