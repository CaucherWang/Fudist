...
#else
                auto result = appr_alg->searchKnnKGraph(massQ + vecdim * i, k, ef);  
#endif
#ifndef WIN32
            GetCurTime( &run_end);
...
int main(int argc, char * argv[]) {
    ...
    #endif
    string result_path_str = result_base_path_str + "/" + data_str + "/" + result_prefix_str + data_str + "_kGraph_K" + to_string(KG) + "_" + exp_name + ".log" + index_postfix + shuf_postfix + query_postfix;
    #ifdef DEEP_DIVE
    ...
    if(data_type == 0){
        ...
        test_performance(Q.data, X.n, Q.n, kgraph, Q.d, answers, subk);
